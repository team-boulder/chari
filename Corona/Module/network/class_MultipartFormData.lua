...
local function networkListener( event )
        if ( event.isError ) then
                tsprint( "Network error!")
        else
                tsprint( "RESPONSE: " .. event.response )
        end
end
                                ...
                                if i ~= #self.elements then
                                    ...
                                    if el.value ~= nil then
                                            if string.find(el.value, "\r\n") == nil then
                                                ...
                                                tsprint( "class_MultipartFormData.lua:  find /r/n" )
                                                el.value = string.gsub(el.value, "\r\n", "")
                                                elData = {
                                                        "--"..self.boundary.."\n",
                                                        "Content-Disposition: form-data; name=\"",
                                                        el.name,
                                        ...
                                        tsprint( "class_MultipartFormData.lua:  el.value == nil" )
                                        elData = {
                                                "--"..self.boundary.."\n",
                                                "Content-Disposition: form-data; name=\"",
                                                el.name,
...
function MultipartFormData:addFile(name, path, mimeType, remoteFileName)
        ...
        local element = {intent="file", name=name, path=path, 
         --        mimetype = mimeType, filename = remoteFileName, encoding = "base64"}
                mimetype = mimeType, filename = remoteFileName, encoding = "binary"}
